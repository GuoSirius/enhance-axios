...
├── src/
│   ├── core/
│   │   ├── index.ts              # 主工厂函数，拦截器编排
│   │   ├── requestManager.ts     # 请求管理器（双 Map）
│   │   ├── tokenManager.ts       # Token 认证（注入、刷新、排队）
│   │   ├── retry.ts              # 失败重试（条件、归一化、延迟）
│   │   ├── dataTransform.ts      # 数据格式转换（Content-Type）
│   │   └── helpers.ts            # 共享工具函数
│   ├── types/
│   │   └── index.ts               # 类型定义
│   ├── utils/
...
│   └── version.ts                # 自动生成的版本号
├── tests/
│   └── index.test.ts             # 测试文件 (95 tests)
├── example/
│   ├── index.html                 # 测试页面 (11 项测试)
│   ├── server.js                  # Mock Server
│   └── mock-api.js               # Mock API (含 /api/echo, /api/token-verify)
├── dist/                          # 构建输出
│   ├── esm/                       # ESM (含 .mjs + .mts)
│   ├── cjs/                       # CJS
│   ├── umd/                       # IIFE (不含 axios)
...
| Token 认证 | 验证 token 注入、过期检测、排队刷新 |
...
| `/api/token-verify` | GET | Token 认证测试 |
